...
<table cellpadding="5" cellspacing="0" id="resultTable" border="1">
  ...
  <tr>
<?php
  $result = $link->query("SELECT grade.idnum,grade.prelim, grade.midterm, grade.midterm, grade.prefinal, grade.final, grade.gwa, ts.*, t.*
...
left join teacher t on t.idnumber=ts.teacher
where grade.idnum='$idnumber' AND ts.section='$section' AND ts.level='$year' AND ts.course='$course'");
while($row = $result->fetch_array())
	{
  ?>
  <tr>
    ...
    <td>
          ...
          while($rows = $results->fetch_array())
			...
			<?php
			//$result = $link->query("SELECT * FROM grade WHERE idnum='$idnumber' AND subject='$subject'");
					//while($row = $result->fetch_array())
						//{
							//echo '<tr>';
                            $stud_gwa= $row['gwa'];

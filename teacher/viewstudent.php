...
$student=clean($_POST['studentid']);
$result = $link->query("SELECT * FROM student WHERE idnumber='$student'");
...
<?php
  ...
  $result = $link->query("SELECT grade.idnum,grade.prelim, grade.midterm, grade.midterm, grade.prefinal, grade.final, grade.gwa, ts.*
...
where ts.teacher='$teacheridnum' and grade.idnum='$idnumber'");
while($row = $result->fetch_array())
	{
  ?>
  <tr>
